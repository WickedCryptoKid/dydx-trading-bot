...
def calculate_half_life(spread):
    ...
    spread_lag = df_spread.spread.shift(1) # shift 1 to above row
...
def calculate_cointegration(series_1, series_2):
  ...
  t_check = coint_t < critical_value # in this model (normally in others not), t value < than critical value
  coint_flag = 1 if p_value < 0.05 and t_check else 0 # if p_value < 0.05 then cointegrated
...
def store_cointegration_results(df_market_prices):
    ...
    for index, base_market in enumerate(markets[:-1]):
        ...
        # Get Quote Pair
        for quote_market in markets[index +1:]: # this loop from the 2nd columns (next to index) to compare w index, and 3rd, and so on
    ...
    # Return results
    print('Cointegrated pairs sucessfully saved')
    return 'saved'